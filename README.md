...
> **防什么**：防用户注销后立即用**同一手机号**重新注册领券。手机号进入 90 天注销沉淀库后，换设备、换 IP 都无法绕过——因为系统比的是手机号哈希（SHA256），不认设备。
...
> **防什么**：防同一台设备**批量注册多个账号**薅新人券。IP 维度 60 秒内请求过频即弹出滑块验证，把自动化脚本卡在人机验证这一步。滑块连败 3 次后 IP 直接封禁，无法继续尝试。
...
> **防什么**：防同一台设备**无限注册新号**（手机号换一批、设备不换）。设备指纹在 MySQL 中追踪活跃账号数，超限直接 403 拒绝——用户必须换一台新设备才能继续注册。
...
> **防什么**：防自动化脚本**故意乱拖滑块**企图绕过验证码（暴力试探、随机偏移）。10 分钟内累计失败 3 次即触发 IP 临时黑名单，封禁时长可在风控面板动态调整（默认 1 分钟测试，生产建议 24 小时）。
...
> **防什么**：防羊毛党**注册→领券→注销→再注册**的循环薅券行为。同一台设备累计注销 2 次即触发设备黑名单（Redis + MySQL 双写，90 天 TTL），此后该设备无论用什么手机号都无法注册。配合场景三（手机号注销库），形成「设备 + 手机号」双维度闭环。
